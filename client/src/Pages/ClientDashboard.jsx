...
const ClientDashboard = () => {
  ...
  const DashboardHome = ({ user, setActiveTab }) => {
    const stats = [
      { icon: <FaCalendarCheck className="text-indigo-600" />, value: "3", label: "Upcoming Sessions", change: "1 tomorrow", onClick: () => setActiveTab("sessions") },
      { icon: <FaComments className="text-indigo-600" />, value: "12", label: "Messages", change: "2 unread", onClick: () => setActiveTab("messages") },
      { icon: <FaFileAlt className="text-indigo-600" />, value: "5", label: "Session History", change: "+1 this week", onClick: () => setActiveTab("history") },
      { icon: <FaMoneyBillWave className="text-indigo-600" />, value: "$150", label: "Pending Payments", change: "Due this week", onClick: () => setActiveTab("payments") },
    ];

    return (
      <div className="space-y-8">
        <motion.div className="bg-gradient-to-r from-indigo-600 to-indigo-500 rounded-xl shadow-lg p-6 text-white" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-2xl font-bold mb-2">Welcome back, {user?.fullName || "Client"}!</h2>
              <p className="opacity-90 max-w-lg">Your personal dashboard to manage sessions, messages, and payments.</p>
      ...
      >
        <div>
          <div className="mb-10 flex flex-col items-start">
            ...
            <p className="text-xs text-gray-500 bg-indigo-50 px-2 py-1 rounded-full">Client Dashboard</p>
          </div>
