...
const Login = () => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/counselor-application"), 1000);
    } catch (error) {
  ...
  const handleForgotPassword = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${backendUrl}/api/counselors/forgot-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleResetPassword = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${backendUrl}/api/counselors/reset-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    ...
    >
      ...
      >
        <img src={Img} alt="Counselor Login" className="w-3/4 h-auto object-contain" />
      </motion.div>
      ...
      <motion.div className="w-full lg:w-1/2 flex justify-center" variants={formVariants} initial="hidden" animate="visible">
        <div className="w-full max-w-md mx-auto text-center bg-white rounded-3xl py-10 lg:py-12 px-6 lg:px-10 shadow-2xl border border-gray-100">
          ...
          >
            {forgotPassword ? "Reset Password" : "Counselor Login"}
          </motion.h2>

          {!forgotPassword ? (
            <form onSubmit={handleLoginSubmit}>
              <motion.div className="relative h-12 w-full mb-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.5 }}>
                <input
                  type="email"
                  name="email"
              ...
              <motion.div className="relative h-12 w-full mb-8" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4, duration: 0.5 }}>
                <input
                  type="password"
                  name="password"
              ...
              <motion.div className="flex flex-col items-center mt-6 space-y-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6, duration: 0.5 }}>
                <p className="text-sm text-gray-600">
                  New Here?{" "}
                  <Link to="/counselor-register" className="text-indigo-600 font-semibold hover:underline transition-all">
                    Sign Up
                  </Link>
                </p>
                <p
            ...
            <form onSubmit={handleResetPassword}>
              ...
              <motion.div className="relative h-12 w-full mb-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4, duration: 0.5 }}>
                <input
                  type="text"
                  value={resetOtp}
                  ...
                  required
                />
                <label className="pointer-events-none absolute left-3 -top-4 flex items-center space-x-2 text-xs font-medium text-gray-800 transition-all peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-500 peer-focus:-top-4 peer-focus:text-xs peer-focus:text-inborder-indigo-600">
                  <FontAwesomeIcon icon={faKey} /> <span>OTP</span>
                </label>
              </motion.div>

              <motion.div className="relative h-12 w-full mb-8" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5, duration: 0.5 }}>
                <input
                  type="password"
                  value={newPassword}
                  onChange={(e) => setNewPassword(e.target.value)}
                  placeholder=""
                  className="peer h-full w-full rounded-xl border border-gray-200 bg-transparent px-12 py-3 text-sm text-gray-700 outline-none transition-all placeholder-shown:border-gray-200 focus:border-2 focus:border-indigo shadow-md"
                  required
                />
                <label className="pointer-events-none absolute left-3 -top-4 flex items-center space-x-2 text-xs font-medium text-gray-800 transition-all peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-500 peer-focus:-top-4 peer-focus:text-xs peer-focus:text-indigo">
              ...
              <motion.button
                type="submit"
                className={`py-3 px-6 rounded-full font-semibold text-white w-2/3 mx-auto flex justify-center items-center bg-gradient-to-r from-indigo to-indigo-500 hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 shadow-md ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
                whileHover={{ scale: loading ? 1 : 1.05 }}
                whileTap={{ scale: loading ? 1 : 0.95 }}
                disabled={loading}
                initial={{ opacity: 0, y: 20 }}
